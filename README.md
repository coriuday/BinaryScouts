...
# Optional: contact form email delivery (Gmail API over HTTPS)
...
Contact and newsletter submissions are saved under `data/contacts/` and `data/newsletter/` (gitignored) and in the admin Leads inbox when Supabase is configured. Email notifications use the **Gmail API** (HTTPS) — required on Render free tier because SMTP ports are blocked. Run `npm run gmail:setup` once to obtain `GMAIL_REFRESH_TOKEN`, then set `GMAIL_USER`, `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, and `CONTACT_TO_EMAIL` on Render.
